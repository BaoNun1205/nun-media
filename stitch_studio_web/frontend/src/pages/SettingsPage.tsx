...
export function SettingsPage() {
  ...
  const [openverseClientId, setOpenverseClientId] = useState('');
  const [openverseClientSecret, setOpenverseClientSecret] = useState('');
  ...
  async function saveOpenverseCredentials(clientId: string, clientSecret: string) {
    try {
      const data = await studioApi.saveSettings({ openverseClientId: clientId, openverseClientSecret: clientSecret });
      setSettings(data); setOpenverseClientId(''); setOpenverseClientSecret(''); setMessage(clientId ? 'Openverse credentials saved. Sound-effect search is ready.' : 'Openverse credentials cleared.');
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to save'); }
  }
  ...
  return (
    <section className="page settings-page">
      ...
      <div className="settings-grid">
        ...
        <section className="settings-card wide"><div className="settings-card-title"><KeyRound size={18} /><div><h2>Openverse credentials</h2><p>Client ID and Client Secret are stored only on the backend. Audio search refreshes OAuth access tokens automatically.</p></div><span className={`status-pill ${settings.hasOpenverseCredentials ? 'completed' : ''}`}>{settings.hasOpenverseCredentials ? <><CheckCircle2 size={13} /> {settings.openverseCredentialsSource === 'environment' ? 'Environment' : 'Stored'}</> : 'Not configured'}</span></div><input type="password" style={{ display: 'block', width: '100%', marginBottom: '8px', background: 'var(--surface-sunken)', border: '1px solid var(--border)', borderRadius: '6px', padding: '12px', color: 'var(--text)', fontSize: '13px' }} value={openverseClientId} onChange={(event) => setOpenverseClientId(event.target.value)} placeholder="Openverse Client ID" autoComplete="off" /><input type="password" style={{ display: 'block', width: '100%', marginBottom: '16px', background: 'var(--surface-sunken)', border: '1px solid var(--border)', borderRadius: '6px', padding: '12px', color: 'var(--text)', fontSize: '13px' }} value={openverseClientSecret} onChange={(event) => setOpenverseClientSecret(event.target.value)} placeholder="Openverse Client Secret" autoComplete="off" /><div className="settings-actions"><button className="primary" disabled={!openverseClientId.trim() || !openverseClientSecret.trim()} onClick={() => saveOpenverseCredentials(openverseClientId, openverseClientSecret)}><Save size={15} /> Save credentials</button><button className="danger" disabled={!settings.hasOpenverseCredentials || settings.openverseCredentialsSource === 'environment'} onClick={() => saveOpenverseCredentials('', '')}><Trash2 size={15} /> Clear</button><span>{message}</span></div></section>
